...
class ResPartner(models.Model):
    ...
    agent = fields.Boolean(
        string="Créancier / Agent",
        help="Check this field if the partner is a creditor or an agent.")
    agent_type = fields.Selection(
        selection=[("agent", "Agent externe")], string="Type", required=True,
        default="agent")
    ...
    settlement = fields.Selection(
        selection=[("monthly", "Mensuel"),
                   ("quaterly", "trimestriel"),
                   ("semi", "Semi-annual"),
                   ("annual", "Annual")],
        string="Période de règlement", default="monthly", required=True)
