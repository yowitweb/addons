...
class WebsiteSale(WebsiteSale):
    def _get_search_domain(self, search, category, attrib_values):
        ...
        if price_vals:
            ...
            if price_vals[0] is not None:
                to_add += [
                    ("default_code", ">=", price_vals[0])
                ]
            print("****************************to_add",to_add)   
            if price_vals[1] is not None:
                to_add += [
                    ("default_code", "<=", price_vals[1])
                ]
            print("****************************to_add1",to_add)   
            if len(to_add) == 1:
                to_add.insert(0, '&')
            domain += to_add
            print("****************************to_add3",to_add)   
        return domain
    ...
    def shop(self, page=0, category=None, search='', ppg=False, **post):
        ...
        # Recherche de prix maximum
        # Utiliser la liste de prix de cette manière pour suivre l'implémentation d'Odoo
        ...
        max_price = product_id.default_code
